...
In computer graphics, shaders play a pivotal role in the rendering pipeline, responsible for various tasks such as vertex manipulation, lighting calculations, and material shading. 
Shaders are small programs executed on the GPU, designed to perform specific tasks during the rendering process. They are written in languages like GLSL (OpenGL Shading Language) or HLSL (High-Level Shading Language) and are essential for achieving realistic graphics.


\subsection{GPU Pipeline}

The GPU pipeline consists of various stages through which geometry and shaders pass to produce the final rendered image. These stages include:

\begin{itemize}
    \item Vertex Shader
    \item Tessellation (Control Shader, Tessellation Shader, Evaluation Shader)
    \item Geometry Shader
    \item Rasterization
    \item Fragment Shader
\end{itemize}

Each stage performs specific operations, such as vertex transformation, tessellation, primitive assembly, and pixel shading. But most of these are optional, the most indispensable ones are Vertex Shader and Frament Shader.



\section{Vertex Shader and Fragment Shader Interaction}

In the graphics rendering pipeline, the Vertex Shader and Fragment Shader play crucial roles in transforming vertices and determining the appearance of fragments (pixels) on the screen, respectively. Understanding how these shaders interact is essential for creating realistic and visually appealing graphics.

\subsection{Vertex Shader}

The Vertex Shader operates on individual vertices of geometric primitives (such as triangles) before they are rasterized into fragments. Its primary task is to transform each vertex from its original coordinate space to a common coordinate space (often referred to as clip space) and to pass along any necessary data to the Fragment Shader.

In addition to position transformation, the Vertex Shader often performs other calculations, such as lighting calculations, texture coordinate generation, and normal transformations. When dealing with lighting, for example, the Vertex Shader might compute per-vertex lighting values based on vertex normals and light positions.

\subsection{Fragment Shader}

The Fragment Shader, also known as the Pixel Shader, operates on each rasterized fragment produced by the rasterization stage. Its main responsibility is to determine the final color of each fragment based on various factors, including lighting, texturing, and surface properties.

The Fragment Shader receives interpolated data from the Vertex Shader, including interpolated vertex attributes such as normals. These interpolated values are computed using barycentric interpolation across the surface of the primitive. For normals specifically, this interpolation ensures that lighting calculations appear smooth across the surface, even though lighting calculations are performed per-vertex in the Vertex Shader.

\subsection{Interpolation of Normals}

In many cases, surfaces in 3D models are represented by flat polygons, meaning that each vertex of the polygon has a single associated normal vector. However, to achieve smooth shading across the surface, normals need to be interpolated across the primitive's surface.

Barycentric interpolation is commonly used to interpolate normals across triangles. Each vertex of the triangle has a corresponding normal vector, and these normals are interpolated across the triangle's surface based on the barycentric coordinates of each fragment within the triangle.

The interpolated normals are then passed from the Vertex Shader to the Fragment Shader, where they are used in lighting calculations to compute the final color of each fragment.

\subsection{Example Interaction}

Consider a scenario where a 3D model of a sphere is rendered. The Vertex Shader transforms the vertices of the sphere and computes normals at each vertex. These normals are then interpolated across the surface of each triangle composing the sphere. In the Fragment Shader, the interpolated normals are used in lighting calculations to determine the brightness of each pixel, resulting in smooth shading on the curved surface of the sphere.


\subsubsection{Vertex Shader}
The vertex shader manipulates vertex positions, normals, textures and passes relevant information to the fragment shader. Below is an example of a vertex shader that applies a simple manipulation to vertex positions for animation effects:
...
\subsubsection{Fragment Shader with Glossy BRDF Calculation}

The fragment shader computes the color of each pixel based on lighting and material properties. Here's an example of a fragment shader that implements a glossy BRDF for simulating surface reflection:
...
void main() {
    ...
    // Combine diffuse and specular terms using Fresnel-Schlick approximation
    // Add equation here
...
\subsubsection{BRDF Equation}

In shader programming, BRDFs are implemented to simulate various material properties such as diffuse reflection, specular highlights, and surface roughness.
A common form of the BRDF equation is the Cook-Torrance model, which combines diffuse and specular reflection components:

\begin{equation}
F(\omega_i, \omega_o) = \frac{F(\omega_i, h) D(h) G(\omega_i, \omega_o, h)}{4(\omega_i \cdot n)(\omega_o \cdot n)}
\end{equation}

where:
\begin{align*}
&F(\omega_i, \omega_o) \text{ is the BRDF}\\
&\omega_i \text{ is the incident light direction}\\
&\omega_o \text{ is the outgoing light direction}\\
&n \text{ is the surface normal}\\
&h \text{ is the half vector between } \omega_i \text{ and } \omega_o\\
&F(\omega_i, h) \text{ is the Fresnel term}\\
&D(h) \text{ is the microfacet distribution function}\\
&G(\omega_i, \omega_o, h) \text{ is the geometric attenuation factor}
\end{align*}
