...
A principal funcionalidade de uma renderizador fotorealista é estimar a radiancia em um ponto $p$ numa dada direção $w_o$. Essa radiancia é dada pela equação de renderização apresentada por @ref Kajiya:

\begin{align*}
  &L_o(p, w_o) = L_e(p, w_o) + 
\int_{H^2}F(p, w_i, w_o){L_i(p,w_i)\cos(\theta)dw_i}\\

    &L_o \text{é radiancia \textit{out}/{observada}}\\
    &L_i
\end{align*}

A Função de Distribuição Bidirecional de Reflectância (BRDF) descreve como a reflexão da luz afeta a radiância de saída. Ela leva em conta vários fatores, como rugosidade da superfície, ângulo de incidência e propriedades do material. Compreender a BRDF permite a modelagem precisa da interação da luz com superfícies na renderização de gráficos computacionais.

Em resumo, a radiometria fornece o quadro fundamental para entender o comportamento da luz em ambientes digitais, essencial para criar renderizações realistas e visualmente atraentes.
