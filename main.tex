...
% \texttt{proximo_token()} recupera o próximo token do fluxo de tokens.
% \texttt{token.prefixo()} é uma função associada ao token que analisa a expressão quando o token é o primeiro token em uma subexpressão.
% \texttt{token.infixo(esquerda)} é uma função associada ao token que analisa a expressão quando o token não é o primeiro token em uma subexpressão, recebendo o operando esquerdo como entrada.
% \texttt{precedencia_anterior} representa a precedência do token anterior, garantindo que os operadores sejam resolvidos na ordem correta com base na precedência.
%
