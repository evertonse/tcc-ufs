...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Não sejam realizadas operações não definidas por \texttt{EquationLang}, como a divisão entre dois vetores. Por exemplo, $\vec{u} / \vec{v}$, mas pode ser válido para um produto escalar ou vetorial.
...
\subsection{Inferencia de Tipos} \label{subsection-type-inference}
...
As declarações de equações são validadas após a coleta e ordenação descritas em \autoref{subsection-sym-resolution}, portanto assume-se que o lado esquerdo das equações seja um identificador válido ou uma definição de função e que estão na ordem correta de avaliação.
...
O controle de visibilidade funciona como esperado, se um parâmetro $x$ existe, ele é preferido ao $x$ global, resultando no fenômeno de \textit{shadowing} ou sombreamento, como exemplificado na equação \autoref{eq-shadowing}, onde o valor de $f$ é 3, não 2.
...
Se um parâmetro $\vec{x}$ é declarado como vetor, todas as operações envolvendo $x$ no corpo da função devem respeitar as operações vetoriais; caso contrário, um erro será automaticamente reportado.
...
No exemplo da \autoref{cod-type-mismatch}, a função $g$ possui a assinatura $\mathbb{R} \times \mathbb{R} \to \mathbb{R}$. A expressão resultante da chamada de função terá o tipo do contradomínio da função. Também é necessário confirmar que $g$ refere-se a um símbolo do tipo função.
