...
As funções e a estrutura de travessia foram utilizadas para validar a precedência dos operadores na AST gerada pelo \texttt{parser}. A função de parentização processa todos os tipos de expressões durante a travessia da AST, inserindo parênteses para preservar a precedência implícita original (sem parênteses). Isso garante que a representação textual reflita corretamente a ordem de avaliação das operações matemáticas, assegurando que a hierarquia das operações foram corretamente representada na árvore gerada.
